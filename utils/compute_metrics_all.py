...
for i, eps in enumerate(eps_list):
    ...
    if eps==EPS_CHOSEN:
        ...
        y = base_pred_y.reshape((1, nsample))
        all_decisions.append(np.any(decisions!=y, axis=0))
        break
...
############################################################################
# elif args.method == 'dropout':
drp_data_filename = [i for i in os.listdir(args.resultroot) if filename_prefix + '-' + args.dropoutmethod + '-' + 'dropout' in i and str(args.drp_nmodel) + '-' + str(args.drp_max_ratio)+'.npz' in i]
...
############################################################################
# elif args.method == 'awp':
awp_data = np.load(args.resultroot + filename_prefix + '-' + 'awp' + '-' + args.awp_eps + '.npz')
...
for i, eps in enumerate(eps_list):
    ...
    if eps==EPS_CHOSEN:
        nsample = decisions[i, :, :].shape[0]
        y = base_pred_y.reshape((nsample,1))
        all_decisions.append(np.any(decisions[i, :, :]!=y, axis=1))
        break
...
print(np.shape(all_decisions))

print(np.mean(all_decisions, axis=1))
print("Overlap between Sampling and dropout")
print(np.mean(np.bitwise_and(all_decisions[0], all_decisions[1])))
print("Overlap between Sampling and AWP")
print(np.mean(np.bitwise_and(all_decisions[0], all_decisions[2])))
print("Overlap between Dropout and AWP")
print(np.mean(np.bitwise_and(all_decisions[1], all_decisions[2])))
exit()
